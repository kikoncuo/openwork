 ...
 * 
 * @param userId - The user ID to scope WhatsApp operations to
 */
export function createWhatsAppTools(userId: string): DynamicStructuredTool[] {
...
IMPORTANT: This tool only works if WhatsApp is connected in Settings > Apps.`,
      ...
      func: async ({ to, message }) => {
        ...
        try {
          ...
          const timestamp = result.timestamp ? new Date(result.timestamp * 1000).toLocaleString() : 'now'
