...
import type { StreamPayload, SDKStreamEvent as StreamEvent, IPCEvent, IPCStreamEvent, Subagent } from '../types'

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type UseStreamTransport = any
...
export class ElectronIPCTransport implements UseStreamTransport {
  ...
  private convertToSDKEvents(event: IPCEvent, threadId: string): StreamEvent[] {
    ...
    switch (event.type) {
      ...
      case 'stream': {
        const streamEvents = this.processStreamEvent(event as IPCStreamEvent)
      ...
      case 'values': {
        const eventData = event.data as { todos?: unknown; files?: unknown; workspacePath?: string; subagents?: Subagent[]; interrupt?: unknown }
        const { todos, files, workspacePath, subagents, interrupt } = eventData
        ...
        if (interrupt) {
          ...
          } else if (typeof interrupt === 'object' && interrupt !== null && 'tool_call' in interrupt) {
            // Legacy format with direct tool_call property
            const legacyInterrupt = interrupt as { id?: string; tool_call: { id: string; name: string; args: Record<string, unknown> } }
            events.push({
              event: 'custom',
              data: {
                type: 'interrupt',
