...
export function ThreadProvider({ children }: { children: ReactNode }) {
  ...
  const loadThreadHistory = useCallback(
    async (threadId: string) => {
      ...
      try {
        const state = await window.api.threads.getState(threadId)

        if (state.messages && Array.isArray(state.messages) && state.messages.length > 0) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const messages: Message[] = (state.messages as any[]).map((msg: {
            ...
            // Handle LangChain serialized format (lc=1, type="constructor")
            if (msg.lc === 1 && msg.type === 'constructor' && Array.isArray(msg.id)) {
              const messageType = msg.id[2] // e.g., "HumanMessage", "AIMessageChunk", "ToolMessage"
              if (messageType === 'HumanMessage') role = 'user'
              else if (messageType === 'AIMessage' || messageType === 'AIMessageChunk') role = 'assistant'
              else if (messageType === 'SystemMessage') role = 'system'
              else if (messageType === 'ToolMessage') role = 'tool'
            ...
            // Extract content from kwargs for LangChain serialized format
            ...
            // Extract other properties from kwargs or directly
            const msgId = msg.kwargs?.id ?? (typeof msg.id === 'string' ? msg.id : undefined) ?? `msg-${index}`
            const toolCalls = msg.kwargs?.tool_calls ?? msg.tool_calls
            const toolCallId = msg.kwargs?.tool_call_id ?? msg.tool_call_id
            const name = msg.kwargs?.name ?? msg.name
        ...
        if (state.todos && Array.isArray(state.todos) && state.todos.length > 0) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const todos: Todo[] = (state.todos as any[]).map((todo: { id?: string; content?: string; status?: string }, index: number) => ({
        ...
        console.error('[ThreadContext] Failed to load thread state:', error)
      }
      ...
      try {
        ...
        console.error('[ThreadContext] Failed to load interrupt state:', error)
      }
    },
    [getThreadActions]
  )
