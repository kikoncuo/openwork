...
export function createAgent(input: CreateAgentInput): Agent {
  ...
  db.run(
    `INSERT INTO agents (agent_id, name, color, icon, model_default, default_workspace_path, is_default, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [agent.agent_id, agent.name, agent.color, agent.icon, agent.model_default, agent.default_workspace_path, agent.is_default, agent.created_at, agent.updated_at]
  )
